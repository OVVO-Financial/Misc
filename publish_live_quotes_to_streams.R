...
repeat{
  ...
  if(!weekend && !holiday){
    if((data.table::hour(start.time)==9 && data.table::minute(start.time)>=30) ||  (data.table::hour(start.time)>=10 && (data.table::hour(start.time)<=16 && data.table::minute(start.time)<=2))){ 
